class LocalData {
  static const mcq = [
    {
      "question": "What is the capital of France?",
      "options": ["Paris", "London", "Berlin", "Madrid"],
      "answer": "Paris"
    },
    {
      "question": "What is the largest planet in our solar system?",
      "options": ["Jupiter", "Saturn", "Neptune", "Uranus"],
      "answer": "Jupiter"
    },
    {
      "question": "Who painted the Mona Lisa?",
      "options": [
        "Leonardo da Vinci",
        "Pablo Picasso",
        "Vincent van Gogh",
        "Michelangelo"
      ],
      "answer": "Leonardo da Vinci"
    },
    {
      "question": "What is the chemical symbol for gold?",
      "options": ["Au", "Ag", "Fe", "Cu"],
      "answer": "Au"
    },
    {
      "question": "What is the tallest mountain in the world?",
      "options": ["Mount Everest", "K2", "Kangchenjunga", "Lhotse"],
      "answer": "Mount Everest"
    },
  ];

  static const mtf = [
    {
      'qus': ['HTTP', 'HTML', 'Git', 'SQL', 'JSON'],
      'ans': [
        'A protocol used for communication on the web.',
        'A markup language for creating web pages.',
        'A version control system used for tracking code changes.',
        'A database query language.',
        'A lightweight data format used for exchanging information.'
      ],
    },
    {
      'qus': [
        'Photosynthesis',
        'Shakespeare',
        'Eiffel Tower',
        'Pacific Ocean',
        'Paris'
      ],
      'ans': [
        'Process by which plants make food',
        'Famous playwright and poet',
        'A landmark in France',
        'Largest ocean on Earth',
        'Capital of France'
      ],
    }
  ];

  static const qus = [
    'Describe the process of photosynthesis.',
    'What are the major causes and effects of global warming?',
    'Explain the significance of the Industrial Revolution.',
    'How does the internet affect modern communication?',
    'What are the advantages and disadvantages of Artificial Intelligence (AI)?',
  ];
}
